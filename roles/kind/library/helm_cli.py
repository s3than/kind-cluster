...
notes:
  - Support for helm2 has been removed
requirements:
  - "helm"
  - "yaml"
  - "tempfile"
...
def get_values(command, release_name, release_namespace):
    get_command = command + " get values --output=yaml " + " --namespace=" + release_namespace + " " + release_name
...
def deploy(command, release_name, release_namespace, release_values, chart_name, wait, wait_timeout, disable_hook, force):
    ...
    if wait:
        ...
        if wait_timeout is not None:
            deploy_command += " --timeout " + str(wait_timeout) + "s"
...
def delete(command, release_name, release_namespace, purge, disable_hook):
...
def main():
    ...
    if release_state == "absent" and release_status is not None:
        helm_cmd = delete(helm_cmd, release_name, release_namespace, purge, disable_hook)
        changed = True
